...
public class YoutubeExtractor {
    ...
    public void parse(String url)
    {
        ...
        try{
            ...
            if(!document.select("meta[property=\"og:restrictions:age\"]").isEmpty())
            {
...
//            playerUrl = getPlayerUrl(playerConfig);
    ...
    private JsonObject getPlayerResponse() throws NullPointerException{
        try {
...
//        if(!streamingData.has(strea))
    ...
    private String decryptionCodeLoad(String playerUrl) {
        try{
            ...
            String decryptionFunctionName = getDecryptionFuncName(player);
    ...
    private String getDecryptionFuncName(String playerCode) {
        ...
        org.schabi.newpipe.extractor.utils.Parser.RegexException exception = null;
        for (String regex : decryptionFuncNameRegexes) {
            try {
                return org.schabi.newpipe.extractor.utils.Parser.matchGroup1(regex, playerCode);
            } catch (org.schabi.newpipe.extractor.utils.Parser.RegexException re) {
                if (exception == null)
                    exception = re;
            }
        }
